...
public class ControllerBuilder implements TemplateHanlder {
    ...
    public void createdManagerImpl(String prefix, List<TableEntity> tables, Map<String, String> config) {
        ...
        String xmlPath = fsPath + projectName + "/";
        String jarPath = config.get("package_name");
        String controllerPath = xmlPath + "src/main/java/" + jarPath.replace(".", "/") + "/controller";
        FileOperate.getInstance().newCreateFolder(controllerPath);
        ...
        if (!Validator.isEmpty(tables)) {
            for (TableEntity table1 : tables) {
                ...
                FileOperate.getInstance()
                        .newCreateFile(controllerPath + tableName + "ManagerImpl.java", result.toString());
            }
        }
    }
